...
|------|-------------|------|---------|:--------:|
| <a name="input_add_firewall_rules"></a> [add\_firewall\_rules](#input\_add\_firewall\_rules) | Controls if extra firewall rules should be added for the cluster | `bool` | `false` | no |
...
| <a name="input_community_plugins"></a> [community\_plugins](#input\_community\_plugins) | List of RabbitMQ COMMUNITY plugins names to enable in your cluster. See list of [supported community plugins](https://www.cloudamqp.com/docs/cloudamqp-plugins.html#community-plugins) | `list(any)` | `[]` | no |
...
| <a name="input_firewall_rules"></a> [firewall\_rules](#input\_firewall\_rules) | Api Key required for integration with log/monitoring service | <pre>list(object({<br/>    ip          = string<br/>    ports       = list(number)<br/>    services    = list(string)<br/>    description = string<br/>  }))</pre> | `[]` | no |
...
| <a name="input_nodes"></a> [nodes](#input\_nodes) | Number of nodes, 1, 3 or 5 depending on plan used. Only needed for legacy plans, will otherwise be computed | `number` | `null` | no |
